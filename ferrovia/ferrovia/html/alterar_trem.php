...
$codigo = $_GET['codigo'] ?? '';
$stmt = $conn->prepare("SELECT * FROM trem WHERE pk_trem = ?");
...
<body>
    ...
    <main>
        <section class="conteudo">
            <form action="editar_trem.php" method="POST"
                onsubmit="return confirm('Deseja realmente salvar as alterações?')">

                <input type="hidden" name="pk_trem" value="<?php echo $sensor["pk_trem"]; ?>">

                <div class="linha-campo">
                    <i class="bi bi-card-heading"></i>
                    <input type="text" name="nome_trem" required value="<?php echo $sensor["nome_trem"]; ?>" />
                </div>

                <div class="linha-campo">
                    <i class="bi bi-calendar-check-fill"></i>
                    <input type="date" name="data_operacao" required
                        value="<?php echo $sensor["data_operacao"]; ?>" />
                </div>
                <div class="linha-campo">
                    <i class="bi bi-person-raised-hand"></i>
                    <input type="text" name="capacidade_trem" required value="<?php echo $sensor["capacidade_trem"]; ?>" />
                </div>
                <div class="linha-campo">
                    <i class="bi bi-speedometer2"></i>
                    <input type="text" name="velocidade_trem" required value="<?php echo $sensor["velocidade_trem"]; ?>" />
                </div>
                <div class="linha-campo">
                    <i class="bi bi-buildings"></i>
                    <input type="text" name="fabricante_trem" required value="<?php echo $sensor["fabricante_trem"]; ?>" />
                </div>
                <div class="linha-campo">
                    <i class="bi bi-eye-fill"></i>
                    <input type="text" name="observacoes_trem" required value="<?php echo $sensor["observacoes_trem"]; ?>" />
                </div>        
                ...
                <div class="linha-campo_botao">
                    ...
                    <div class="botao_cancelar">
                        <a href="trem.php"><input class="botao_excluir" type="button" value="Cancelar"></a>
                    </div>
